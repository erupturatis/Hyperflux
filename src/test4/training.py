...
from src.test4.network_conv6 import ModelCifar10Conv6
import numpy as np
from torch.optim.lr_scheduler import StepLR


def train(model:ModelCifar10Conv6, train_loader, optimizer, epoch, prune_enabled = True):
...
def run_conv6():
    ...
    batch_size = 128
    lr_weight_bias = 0.0007
    lr_custom_params = 0.005
    num_epochs = 100
    ...
    model = ModelCifar10Conv6(configs_network_masks).to(get_device())
    ...
    optimizer = torch.optim.Adam([
                        {'params': weight_bias_params, 'lr': lr_weight_bias, 'weight_decay' : 1e-4},
                        {'params': custom_params, 'lr': lr_custom_params},
    ])

    lambda_lr_weight_bias = lambda epoch: 1 ** (epoch // 2)
    lambda_lr_custom_params = lambda epoch: 1.2 ** (epoch // 2)
    scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=[lambda_lr_weight_bias, lambda_lr_custom_params])

    for epoch in range(1, num_epochs + 1):
        train(model, train_loader, optimizer, epoch, configs_network_masks['mask_pruning_enabled'])
