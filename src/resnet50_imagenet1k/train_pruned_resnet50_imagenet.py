...
def initialize_model():
    ...
    if "resume" in training_configs:
        MODEL.load(training_configs["resume"], BASELINE_RESNET50_IMAGENET)
...
#
#     scheduler_weights_lr_during_pruning = CosineAnnealingLR(training_context.get_optimizer_weights(), T_max=pruning_end, eta_min=1e-2 / 2)
...
def initialize_training_context():
    ...
    training_context = TrainingContextPrunedTrain(
        TrainingContextPrunedTrainArgs(
            ...
            lr_flow_params_reset=get_lr_flow_params() * training_configs["reset_lr_flow_params_scaler"],
            l0_gamma_scaler=0,
            optimizer_weights=optimizer_weights,
            optimizer_flow_mask=optimizer_flow_mask
        )
...
def initialize_stages_context():
    ...
    pruning_scheduler = PressureScheduler(pressure_exponent_constant=1.5, sparsity_target=training_configs["target_sparsity"], epochs_target=pruning_end, step_size=0.15)
    ...
    scheduler_weights_lr_during_regrowth = CosineAnnealingLR(training_context.get_optimizer_weights(), T_max=regrowth_stage_length, eta_min=training_configs["end_lr_regrowth"])
...
def train_resnet50_imagenet_sparse_model(sparsity_configs_aux):
    ...
    for epoch in range(1, stages_context.args.regrowth_epoch_end + 1):
        ...
        stages_context.update_context(epoch_global, get_model_sparsity_percent(MODEL_MODULE))
        stages_context.step(training_context)
        if epoch % 5 == 1:
            MODEL_MODULE.save(f"/resnet50_imagenet_sparsity{get_model_sparsity_percent(MODEL_MODULE)}_acc{acc}_{epoch}")
            MODEL_MODULE.save_entire_dict(f"/resnet50_entire_imagenet_sparsity{get_model_sparsity_percent(MODEL_MODULE)}_acc{acc}_{epoch}")
    

    MODEL_MODULE.save(f"/resnet50_imagenet_sparsity{get_model_sparsity_percent(MODEL_MODULE)}_acc{acc}")
