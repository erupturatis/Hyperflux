...
def initialize_stages_context():
    ...
    pruning_scheduler = PressureSchedulerPolicy1(pressure_exponent_constant=1.5, sparsity_target=training_configs["target_sparsity"], epochs_target=pruning_end, step_size=0.15)
...
def train_resnet50_imagenet_sparse_model(sparsity_configs_aux: TrainingConfigsWithResume):
    ...
    for epoch in range(1, stages_context.args.regrowth_epoch_end + 1):
        ...
        stages_context.update_context(epoch_global, get_custom_model_sparsity_percent(MODEL_MODULE))
    ...
    MODEL_MODULE.save(f"/resnet50_imagenet_sparsity{get_custom_model_sparsity_percent(MODEL_MODULE)}_acc{acc}")
