...
def initialize_training_context():
    ...
    training_context = TrainingContextPrunedTrain(
        TrainingContextPrunedTrainArgs(
            lr_weights_reset=lr_weights_initial / 10, # second LR
...
def initialize_stages_context():
    ...
    scheduler_weights_lr_during_regrowth = torch.optim.lr_scheduler.MultiStepLR(training_context.get_optimizer_weights(), milestones=[25], last_epoch=-1)
    # after 25 epochs regrowing stops completely at epoch 150
    scheduler_flow_params_lr_during_regrowth = LambdaLR(training_context.get_optimizer_flow_mask(), lr_lambda=lambda iter: scheduler_decay_after_pruning ** iter if iter < 25 else 0)

    stages_context = StagesContextPrunedTrain(
        StagesContextPrunedTrainArgs(
            pruning_epoch_end=pruning_end,
            regrowth_epoch_end=training_end,
...
def train_resnet50_cifar10_sparse_model_multistep():
