...
from src.resnet18_cifar100.resnet18_cifar100_attributes import RESNET18_CIFAR100_REGISTERED_LAYERS_ATTRIBUTES, \
...
def forward_pass_resnet18_cifar100(self: 'LayerComposite', x: torch.Tensor) -> torch.Tensor:
    # Ensures all layers used in forward are registered in these 2 arrays
    registered_layers_object = SimpleNamespace()
    for layer in RESNET18_CIFAR100_REGISTERED_LAYERS_ATTRIBUTES:
        name = layer['name']
        layer = getattr(self, name)
        setattr(registered_layers_object, name, layer)

    unregistered_layers_object = SimpleNamespace()
    for layer in RESNET18_CIFAR100_UNREGISTERED_LAYERS_ATTRIBUTES:
        name = layer['name']
        layer = getattr(self, name)
