...
def forward_pass_vgg19_cifar10(self: 'LayerComposite', x: torch.Tensor) -> torch.Tensor:
    # Ensure all layers used in forward are registered
    registered_layers_object = SimpleNamespace()
    for layer in VGG19_CIFAR10_REGISTERED_LAYERS_ATTRIBUTES:
        name = layer['name']
