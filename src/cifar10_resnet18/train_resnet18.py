...
def run_cifar10_resnet18():
    ...
    optimizer_weights = torch.optim.AdamW(weight_bias_params, lr=lr_weight_bias, eps = 1e-8, weight_decay=0.01)
    scheduler_weights = CosineAnnealingWarmRestarts(optimizer_weights, T_0=50, T_mult=2, eta_min=1e-6)
    ...
    scheduler_decay_after_pruning = 0.98
    ...
    scheduler_flipping = LambdaLR(optimizer_flipping, lr_lambda=lambda epoch: 1 if epoch < stop_epoch else 1 * scheduler_decay_after_pruning ** (epoch - stop_epoch))
