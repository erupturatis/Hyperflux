...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        grid = (self.img_size[0] // self.patch_size[0], self.img_size[1] // self.patch_size[1])
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        assert (H, W) == self.img_size, f"Input size ({H}x{W}) != model ({self.img_size[0]}x{self.img_size[1]})"
...
class PrunableAttention(nn.Module):
    def __init__(
        ...
        self.qkv = LayerLinearMaskImportance(
            ConfigsLayerLinear(in_features=dim, out_features=3 * dim, bias_enabled=qkv_bias),
            configs_network_masks
        )
...
class PrunableBlock(nn.Module):
    def __init__(
        ...
        self.attn = PrunableAttention(dim, num_heads, qkv_bias, attn_drop, drop, configs_network_masks)
...
class VisionTransformerPrunable(LayerComposite):
    def __init__(
        self,
        configs_network_masks,                # your ConfigsNetworkMasksImportance
        img_size=224,
        patch_size=16,
        in_chans=3,
        num_classes=1000,
        ...
        # Blocks with linearly increasing drop_path
        dpr = torch.linspace(0, drop_path_rate, steps=depth).tolist()
        self.blocks = nn.ModuleList([
            PrunableBlock(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, drop=drop_rate, attn_drop=attn_drop_rate,
                drop_path=dpr[i], norm_layer=norm_layer, configs_network_masks=configs_network_masks
            )
    ...
    def _patch_forward(self, x):
        ...
        # prunable conv path
        B = x.shape[0]
        x = self.patch_embed(x)  # conv2d via LayerConv2MaskImportance -> [B, D, H/ps, W/ps]
...
def deit_tiny_prunable(configs_network_masks, **kwargs):
    return VisionTransformerPrunable(
        ...
        patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4.0,
...
def deit_small_prunable(configs_network_masks, **kwargs):
    return VisionTransformerPrunable(
        configs_network_masks=configs_network_masks,
        patch_size=16, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4.0,
        qkv_bias=True, **kwargs
    )
...
if __name__ == "__main__":
    from src.infrastructure.layers import ConfigsNetworkMasksImportance  # TypedDict in your code
    cfg = ConfigsNetworkMasksImportance(mask_pruning_enabled=True, weights_training_enabled=True)
    model = deit_small_prunable(cfg, img_size=224, in_chans=3, num_classes=1000, drop_path_rate=0.1)
    x = torch.randn(2, 3, 224, 224)
    y = model(x)
    print("logits:", y.shape)
    # verify prunable layers are registered
    print("registered prunable layers:", len(model.registered_layers))
