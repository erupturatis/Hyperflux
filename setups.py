from src.mnist_lenet300.train_pruned_lenet300_mnist_adam import train_pruned_lenet300_mnist_adam
from src.mnist_lenet300.train_sparsity_curves_adam import run_lenet300_mnist_adam_sparsity_curve
from src.mnist_lenet300.train_sparsity_curves_fixed_lr import run_lenet300_mnist_adam_sparsity_curve_lrs
from src.infrastructure.constants import PRUNED_MODELS_PATH
from src.resnet50_cifar10.train_pruned_resnet50_cifar10 import train_resnet50_cifar10_sparse_model
from src.resnet50_cifar10.train_scratch_resnet50_cifar10 import train_resnet50_cifar10_from_scratch_multistep
from src.resnet50_cifar100.train_pruned_resnet50_cifar100 import train_resnet50_cifar100_sparse_model
from src.resnet50_cifar100.train_scratch_resnet50_cifar100 import \
    train_resnet50_cifar100_from_scratch_multistep
from src.resnet50_imagenet1k.train_pruned_resnet50_imagenet import train_resnet50_imagenet_sparse_model
from src.vgg19_cifar10.train_pruned_vgg19_cifar10 import train_vgg19_cifar10_sparse_model
from src.vgg19_cifar10.train_scratch_vgg19_cifar10 import train_vgg19_cifar10_from_scratch_multistep
from src.vgg19_cifar100.train_pruned_vgg19_cifar100 import train_vgg19_cifar100_sparse_model
from src.vgg19_cifar100.train_scratch_vgg19_cifar100 import train_vgg19_cifar100_from_scratch_multistep

INITIAL_LR = 0.1
HIGH_LR = 0.01
MID_LR = 0.001
LOW_LR = 0.0001

##################################################################################################################
# USES INITIAL LR INSTEAD AT THE BEGINNING
##################################################################################################################

def vgg19_cifar100_setup_stable_version2(target_sparsity):
    remaining = 100 - target_sparsity
    target_before_regrowth = remaining * 10 / 13 # offsets regrowth
    train_vgg19_cifar100_sparse_model(
        sparsity_configs_aux={
            "pruning_end":100,
            "regrowing_end":160,
            "target_sparsity": target_before_regrowth,
            "lr_flow_params_decay_regrowing": 0.75,
            "start_lr_pruning": INITIAL_LR,
            "end_lr_pruning": HIGH_LR/3,
            "reset_lr_pruning": MID_LR,
            "end_lr_regrowth": LOW_LR,
            "reset_lr_flow_params_scaler": 5,
            "weight_decay": 5e-4,
            "resume": "vgg19_cifar100_accuracy73.51%",
            "notes": '''
            '''
        }
    )

def vgg19_cifar10_setup_stable_version2(target_sparsity):
    remaining = 100 - target_sparsity
    target_before_regrowth = remaining * 11.5 / 13 # offsets regrowth

    train_vgg19_cifar10_sparse_model(
        sparsity_configs_aux={
            "pruning_end":100,
            "regrowing_end":160,
            "target_sparsity": target_before_regrowth,
            "lr_flow_params_decay_regrowing": 0.75,
            "start_lr_pruning": INITIAL_LR,
            "end_lr_pruning": HIGH_LR/3,
            "reset_lr_pruning": MID_LR,
            "end_lr_regrowth": LOW_LR,
            "reset_lr_flow_params_scaler": 5,
            "weight_decay": 5e-4,
            "resume": "vgg19_cifar10_accuracy93.5%",
            "notes": '''
            '''
        }
    )

def resnet50_cifar100_stable_version2(target_sparsity):
    remaining = 100 - target_sparsity
    target_before_regrowth = remaining * 11.5 / 13 # offsets regrowth
    train_resnet50_cifar100_sparse_model(
        sparsity_configs_aux={
            "pruning_end":100,
            "regrowing_end":160,
            "target_sparsity": target_before_regrowth,
            "lr_flow_params_decay_regrowing": 0.75,
            "start_lr_pruning": INITIAL_LR,
            "end_lr_pruning": HIGH_LR/3,
            "reset_lr_pruning": MID_LR,
            "end_lr_regrowth": LOW_LR,
            "reset_lr_flow_params_scaler": 5,
            "weight_decay": 5e-4,
            "resume": "resnet50_cifar100_accuracy78.54%_multistep",
            "notes": '''
            '''
        }
    )

def resnet50_cifar10_stable_version2(target_sparsity):
    remaining = 100 - target_sparsity
    target_before_regrowth = remaining * 11.5 / 13 # offsets regrowth
    train_resnet50_cifar10_sparse_model(
        sparsity_configs_aux={
            "pruning_end":100,
            "regrowing_end":160,
            "target_sparsity": target_before_regrowth,
            "lr_flow_params_decay_regrowing": 0.75,
            "start_lr_pruning": INITIAL_LR,
            "end_lr_pruning": HIGH_LR/3,
            "reset_lr_pruning": MID_LR,
            "end_lr_regrowth": LOW_LR,
            "reset_lr_flow_params_scaler": 5,
            "weight_decay": 5e-4,
            "resume": "resnet50_cifar10_accuracy94.64%",
            "notes": '''
            '''
        }
    )

##################################################################################################################
# USES HIGH LR INSTEAD AT THE BEGINNING
##################################################################################################################

def resnet50_cifar100_stable_version1(target_sparsity):
    remaining = 100 - target_sparsity
    target_before_regrowth = remaining * 11 / 13 # offsets regrowth
    train_resnet50_cifar100_sparse_model(
        sparsity_configs_aux={
            "pruning_end":100,
            "regrowing_end":160,
            "target_sparsity": target_before_regrowth,
            "lr_flow_params_decay_regrowing": 0.8,
            "start_lr_pruning": 2*HIGH_LR,
            "end_lr_pruning": HIGH_LR/3,
            "reset_lr_pruning": HIGH_LR/10,
            "end_lr_regrowth": LOW_LR,
            "reset_lr_flow_params_scaler": 5,
            "weight_decay": 5e-4,
            "resume": "resnet50_cifar100_accuracy78.54%_multistep",
            "notes": '''
            testing overnight
            '''
        }
    )

def resnet50_cifar10_stable_version1(target_sparsity):
    remaining = 100 - target_sparsity
    target_before_regrowth = remaining * 11 / 13 # offsets regrowth
    train_resnet50_cifar10_sparse_model(
        sparsity_configs_aux={
            "pruning_end":100,
            "regrowing_end":160,
            "target_sparsity": target_before_regrowth,
            "lr_flow_params_decay_regrowing": 0.9,
            "start_lr_pruning": 2*HIGH_LR,
            "end_lr_pruning": HIGH_LR/3,
            "reset_lr_pruning": HIGH_LR/10,
            "end_lr_regrowth": LOW_LR,
            "reset_lr_flow_params_scaler": 5,
            "weight_decay": 5e-4,
            "resume": "resnet50_cifar10_accuracy94.64%",
            "notes": '''
            testing overnight
            '''
        }
    )

def resnet50_imagenet_stable_version1(target_sparsity):
    remaining = 100 - target_sparsity
    target_before_regrowth = remaining * 11 / 13 # offsets regrowth
    train_resnet50_imagenet_sparse_model(
        sparsity_configs_aux={
            "pruning_end": 90,
            "regrowing_end": 150,
            "target_sparsity": target_before_regrowth,
            "lr_flow_params_decay_regrowing": 0.75,
            "start_lr_pruning": HIGH_LR,
            "end_lr_pruning": HIGH_LR/3,
            "reset_lr_pruning": HIGH_LR/10,
            "end_lr_regrowth": LOW_LR,
            "reset_lr_flow_params_scaler": 3,
            "weight_decay": 1e-4,
            "resume": "baseline_imagenet.pth",
            "notes": '''
            testing overnight
            '''
        }
    )

def vgg19_cifar100_setup_stable_version1(target_sparsity):
    remaining = 100 - target_sparsity
    target_before_regrowth = remaining * 11 / 13 # offsets regrowth
    train_vgg19_cifar100_sparse_model(
        sparsity_configs_aux={
            "pruning_end":100,
            "regrowing_end":160,
            "target_sparsity": target_before_regrowth,
            "lr_flow_params_decay_regrowing": 0.8,
            "start_lr_pruning": 2*HIGH_LR,
            "end_lr_pruning": HIGH_LR/3,
            "reset_lr_pruning": HIGH_LR/10,
            "end_lr_regrowth": LOW_LR,
            "reset_lr_flow_params_scaler": 5,
            "weight_decay": 5e-4,
            "resume": "vgg19_cifar100_accuracy73.51%",
            "notes": '''
            testing overnight
            '''
        }
    )


def vgg19_cifar10_setup_stable_version1(target_sparsity):
    remaining = 100 - target_sparsity
    target_before_regrowth = remaining * 10 / 13 # offsets regrowth
    train_vgg19_cifar10_sparse_model(
        sparsity_configs_aux={
            "pruning_end":100,
            "regrowing_end":160,
            "target_sparsity": target_before_regrowth,
            "lr_flow_params_decay_regrowing": 0.8,
            "start_lr_pruning": 2*HIGH_LR,
            "end_lr_pruning": HIGH_LR/3,
            "reset_lr_pruning": HIGH_LR/10,
            "end_lr_regrowth": LOW_LR,
            "reset_lr_flow_params_scaler": 5,
            "weight_decay": 5e-4,
            "resume": "vgg19_cifar10_accuracy93.5%",
            "notes": '''
            testing overnight
            '''
        }
    )
##################################################################################################################
##################################################################################################################
